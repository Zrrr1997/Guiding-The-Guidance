...
def save_nifti(name, im):
    ...
    ni_img.to_filename(f'data/{name}.nii.gz')
...
class AddGuidanceSignalDeepEditd(MapTransform):
    ...
    def __init__(
        ...
        #self.sigma = 0 if self.adaptive_sigma else sigma
    def _get_sigma(self, geos_mean):
        if self.spacing[0] == 2.03642011: # AutoPET
            sigmas = {13: 0.03035366, 9: 0.02231677, 5: 0.01640785, 1: 0.01206346, 0: 0.00886936}
        else:
            sigmas = {13: 0.05183595, 9: 0.04843885, 5: 0.04526439, 1: 0.04229796, 0: 0.03952595}


        for el in sigmas:
            if geos_mean > sigmas[el]:
                return el
        return 0
            ...
            if torch.max(signal[0]) > 0:
                ...
                if self.disks:
                    ...
                    if self.gdt or self.edt or self.adaptive_sigma:
                        ...
                        signal_tensor = signal[0] * 0
                        signal[0] = geos[0][0]
                        if self.adaptive_sigma:
                            geos = geos.cpu().detach().numpy()
                            print(guidance)
                            #guidance = [ [1, 172, 115, 32],  [1, 202, 147, 32], [1, 192, 110, 32],  [1, 206, 113, 32]]
                            #sigmas = [1, 1, 13, 3]
                            for ind, point in enumerate(guidance):
                                new_disk = signal_tensor * 0
                                if torch.any(torch.asarray(point) < 0):
                                    continue
                                # Making sure points fall inside the image dimension
                                p1 = max(0, min(int(point[-3]), sshape[-3] - 1))
                                p2 = max(0, min(int(point[-2]), sshape[-2] - 1))
                                p3 = max(0, min(int(point[-1]), sshape[-1] - 1))
                                new_disk[p1, p2, p3] = 1.0


                                cw = 9
                                geos_context = geos[0, 0, p1 - cw: p1 + cw, p2 - cw: p2 + cw, p3 - cw: p3 + cw]

                                if len(geos_context[geos_context > 0]) == 0:
                                    sigma = 13
                                else:
                                    geos_mean = np.mean(geos_context)
                                    sigma = self._get_sigma(geos_mean)
                                sigma = 1 if key_label == 'background' else sigma
                                if key_label != 'background':
                                    print('Sigma', sigma)
                                pt_gaussian = GaussianFilter(len(new_disk.shape), sigma=sigma)
                                new_disk = pt_gaussian(new_disk.clone().detach().unsqueeze(0).unsqueeze(0))
                                new_disk = (new_disk - torch.min(new_disk)) / (torch.max(new_disk) - torch.min(new_disk))

                                '''
                                # Initial values to compute the interval thresholds
                                if key_label == 'spleen':
                                    if not os.path.exists(f'./data/msd_statistics/{ind}_geos.npy'):
                                        x = np.array([])
                                        np.save(f'./data/msd_statistics/{ind}_geos.npy', x)
                                    else:
                                        x = list(np.load(f'./data/msd_statistics/{ind}_geos.npy'))

                                        x.append(np.mean(geos_context[geos_context > 0]) / np.max(geos))
                                        np.save(f'./data/msd_statistics/{ind}_geos.npy', x)
                                    #print('Var context', round(np.mean(geos_context[geos_context > 0]) / np.max(geos), 3))
                                '''


                                signal_tensor += new_disk.squeeze(0).squeeze(0)
                            signal[0] = (signal_tensor - torch.min(signal_tensor)) / (torch.max(signal_tensor) - torch.min(signal_tensor))
                            #signal[0] = (signal_tensor > 0.1) * 1.0 # 0.1 with sigma=1 --> radius = 3, otherwise it is a cube
        ...
        for key in self.key_iterator(d):
            if key == "image":
                ...
                for key_label in guidance.keys():
                    ...
                    #tmp_image = torch.cat([tmp_image, signal.to(self.device)], dim=0)
                    if isinstance(d[key], MetaTensor):
                        d[key].array = tmp_image
                    else:
                        d[key] = tmp_image
    ...
    def __call__(self, data: Mapping[Hashable, np.ndarray]) -> Dict[Hashable, np.ndarray]:
        ...
        for key in self.key_iterator(d):
            if key == "label":
                ...
                #print(487, d[self.guidance], label_guidances)
        ...
        if self._will_interact:
            ...
            while True:
                ...
                if len(keep_guidance) == len(d["label_names"].keys()):
                    ...
                    break
            '''
        #d[self.guidance_key] = self.guidance  # Update the guidance (basically overwrites it)
        #print(710, d[self.guidance_key], self.guidance)
...
class AddGuidanceFromPointsDeepEditd(Transform):
    ...
    def __call__(self, data):
        ...
        d[self.guidance] = update_guidance(d[self.guidance], all_guidances)
...
class ResizeGuidanceMultipleLabelDeepEditd(Transform):
    ...
    def __call__(self, data):
        ...
        for key_label in d[self.guidance].keys():
            ...
            if not len(d[self.guidance][key_label]):
                all_guidances[key_label] = guidance

        #d[self.guidance] = all_guidances
        #print(815, d[self.guidance], all_guidances)
        d[self.guidance] = update_guidance(d[self.guidance], all_guidances)
        return d
    ...
    def __call__(self, data: Mapping[Hashable, np.ndarray]) -> Dict[Hashable, np.ndarray]:
        ...
        for key in self.key_iterator(d):
            if key == "label":
                ...
                #print('update 921', d[self.guidance], label_guidances)
