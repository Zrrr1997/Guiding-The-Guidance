...
class DiceCE_Rec_Loss(_Loss):
    ...
    def forward(self, input_ct_pet: torch.Tensor, target_ct_seg: torch.Tensor) -> torch.Tensor:
        ...
        input = input_ct_pet[:,:2] # Seg_pred

        target = target_ct_seg # Take only SEG_gt data



        input_rec = input_ct_pet[:,2:3] # PET rec
        target_rec = input_ct_pet[:,3:4] # PET-raw
        ...
        #   self.lambda_rec * rec_loss + self.lambda_dice * ce_loss +
