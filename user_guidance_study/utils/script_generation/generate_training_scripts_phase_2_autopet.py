...
def inp_enc(x):
    if x == 'conv1d':
        ...
        return ' '
name = 'name'
base_script = f'python train_autopet.py --output output_autopet/name/ --cache_dir cache_autopet/name/ --sigma SIGMA_VAL xxx -dpt PROB'
...
for p in ['0.5', '0.75', '1.0']:
    for enc in ['concat', 'conv1s', 'conv1d']:
        # Heatmaps
        name = f'heatmaps_sigma=0_p={p}_enc={enc}'
        ...
        name = f'disks_sigma=0_p={p}_enc={enc}'
        ...
        name = f'edt_sigma=1_p={p}_enc={enc}_gdt-theta=50'
        script_edt = script_edt.replace('name', name)
        script_edt = script_edt.replace('SIGMA_VAL', '1')


        name = f'gdt_sigma=5_p={p}_enc={enc}_gdt-theta=10'
