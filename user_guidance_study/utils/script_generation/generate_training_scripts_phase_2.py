...
def inp_enc(x):
    if x == 'conv1d':
        ...
        return ' '
name = 'name'
base_script = f'python train.py --output output/name/ --cache_dir cache/name/ --sigma SIGMA_VAL xxx -dpt PROB'
...
for p in ['0.5', '0.75', '1.0']:
    for enc in ['concat', 'conv1s', 'conv1d']:
        ...
        name = f'disks_sigma=1_p={p}_enc={enc}'
        script_disks = script_disks.replace('name', name)
        script_disks = script_disks.replace('SIGMA_VAL', '1')
        ...
        script_gdt = script_1 + f' --gdt true --disks true --gdt_th 30'
        script_edt = script_1 + f' --edt true --disks true --gdt_th 10'

        name = f'edt_sigma=1_p={p}_enc={enc}_gdt-theta=10'
        script_edt = script_edt.replace('name', name)
        script_edt = script_edt.replace('SIGMA_VAL', '1')


        name = f'gdt_sigma=5_p={p}_enc={enc}_gdt-theta=30'
...
with open ('scripts_phase_2.txt', 'w') as f:
