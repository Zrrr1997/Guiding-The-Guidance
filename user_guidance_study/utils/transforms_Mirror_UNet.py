...
class AddGuidanceSignalDeepEditd(MapTransform):
    ...
    def __init__(
        ...
        device = None,
        spacing = None,
        geos_image_seed = None,
    ):
        super().__init__(keys, allow_missing_keys)
        self.guidance = guidance
        self.sigma = sigma
        self.number_intensity_ch = number_intensity_ch # Current channels to concatenate on top of
        self.disks = disks
        self.edt = edt
        self.gdt = gdt
        self.gdt_th = gdt_th
        ...
        self.geos_image_seed = geos_image_seed

        self.gdt_th = 0 if self.exp_geos else self.gdt_th
        self.gdt = True if self.exp_geos else self.gdt

        self.sigma = 5 if self.gdt else 0


    def _get_signal(self, image_, guidance, last_discrepancy):

        if self.guidance == 'guidance_ct' or self.geos_image_seed == 'ct':
            image = image_[:1] # CT
        else:
            image = image_[1:2] # PET for "guidance" in exp_1 and exp_2

        dimensions = 3 if len(image.shape) > 3 else 2
        guidance = guidance.tolist() if isinstance(guidance, np.ndarray) else guidance
        guidance = json.loads(guidance) if isinstance(guidance, str) else guidance

        if (self.gdt or self.edt) and self.guidance == 'guidance_ct':
            assert self.disks
        ...
        if len(guidance):
            ...
            # Apply a Gaussian filter to the signal (only if there are any clicks)
            if torch.max(signal[0]) > 0:
                ...
                if self.disks:
                    ...
                    if self.gdt or self.edt:
                        fact = 1.0 if (self.gdt or self.exp_geos) else 0.0
    ...
    def __call__(self, data: Mapping[Hashable, np.ndarray]) -> Dict[Hashable, np.ndarray]:
        ...
        if 'last_discrepancy' in d.keys():
            last_discrepancy = d['last_discrepancy']
        for key in self.key_iterator(d):
            if key == "image":
                ...
                for key_label in guidance.keys():
                    ...
                    if guidance[key_label] is not None and len(guidance[key_label]):
                        signal = self._get_signal(image, guidance[key_label], last_discrepancy)
                    else:
                        signal = self._get_signal(image, [], last_discrepancy)
                ...
                for _, (key_label, val_label) in enumerate(d["label_names"].items()):
                    if key_label != "background":
                        ...
                        if self.pred == 'pred':
                            ...
                            if self.args.guidance_exp == 'exp_1':
                                # Sample for PET clicks
                                if np.random.choice([True, False], p=[self.args.PET_prob, 1 - self.args.PET_prob]):
                                    pred = np.copy(d[self.pred][2:]) # Take only PET prediction
                                    d['last_discrepancy'] = 'PET'
                                else:
                                    pred = np.copy(d[self.pred][:2]) # Take only CT prediction
                                    d['last_discrepancy'] = 'CT'

                            elif self.args.guidance_exp in ['exp_2', 'exp_4']:
                                # Sample from fused prediction
                                pred = np.copy(d['pred']) # Use fused pred not the concatenated one
                            elif self.args.guidance_exp == 'exp_3':
                                # Sample CT and PET-specific Clicks
                                if self.discrepancy == 'discrepancy_ct':
                                    pred = np.copy(d[self.pred][:2]) # Take only CT prediction
                                else: # 'discrepancy_pet'
                                    pred = np.copy(d[self.pred][2:]) # Take only CT prediction
...
class AddRandomGuidanceDeepEditd(Randomizable, MapTransform):
    ...
    def find_guidance(self, discrepancy):
        ...
        if np.mean(discrepancy) > 0.99:
            distance = discrepancy / np.mean(discrepancy)
